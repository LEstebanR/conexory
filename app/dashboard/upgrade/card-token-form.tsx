...
// subscription's card: cardNumber, expirationDate and securityCode are
// mounted as Mercado Pago iframes we never touch, so raw card data never
// reaches our server — only the resulting token does. Identification type
// options are fetched and populated by the SDK itself once mounted (varies
// by country). The SDK loads in the background from mount so the fields are
// ready the moment the buyer reveals them, but the fields themselves stay
// hidden until then — showing a card number/CVV/document form immediately
...
export function CardTokenForm({
  ...
  }, [email, amount, onTokenize, onSuccess, onFailure, errorMessage])
