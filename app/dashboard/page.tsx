...
import { Plus, Building2, LinkIcon, MapPin, BedDouble, Bath, Square, Share2 } from "lucide-react"
...
function PropertyCard({ property }: { property: Property }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 hover:border-slate-200 transition-colors flex flex-col gap-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <span className="inline-flex items-center bg-brand-50 text-brand-700 text-[10px] font-bold px-2 py-1 rounded-full mb-2">
            {typeLabel} · En venta
          </span>
          <h3 className="font-black text-slate-950 tracking-tight leading-tight truncate">
            {property.title}
          </h3>
          {location && (
        ...
        <p className="text-lg font-black text-slate-950 tracking-tighter flex-shrink-0">{price}</p>
      </div>
      ...
      {(property.area != null || property.bedrooms != null || property.bathrooms != null) && (
        <div className="flex items-center gap-4">
          ...
          {property.bedrooms != null && (
            <div className="flex items-center gap-1 text-xs text-slate-500 font-medium">
              <BedDouble className="w-3.5 h-3.5 text-slate-400" strokeWidth={1.75} />
              {property.bedrooms} hab.
            </div>
          )}
          {property.bathrooms != null && (
            <div className="flex items-center gap-1 text-xs text-slate-500 font-medium">
              <Bath className="w-3.5 h-3.5 text-slate-400" strokeWidth={1.75} />
      ...
      <div className="flex gap-2 pt-1 border-t border-slate-50">
        <Link
          ...
          className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-xl bg-brand-50 text-brand-700 text-xs font-bold hover:bg-brand-100 transition-colors"
        >
        ...
        <Link
          ...
          className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-slate-100 transition-colors"
        >
...
export default async function DashboardPage() {
  ...
  const statCards = [
    { label: "Propiedades activas", value: count, icon: Building2, color: "text-brand-500", bg: "bg-brand-50" },
