...
export default function FlyerModal({
  ...
  const [selectedImages, setSelectedImages] = useState<string[]>(images.slice(0, 4))
  ...
  function toggleImage(url: string) {
    setPreviewUrl(null)
    setPreviewError(false)
    setSelectedImages((prev) => prev.includes(url) ? (prev.length === 1 ? prev : prev.filter((item) => item !== url)) : prev.length >= 6 ? prev : [...prev, url])
  }
              ...
              <div className="min-w-0 flex flex-col space-y-7">
                <section ref={mobilePreviewRef} className="order-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
                    {FLYER_TEMPLATE_IDS.map((id) => <button key={id} type="button" onClick={() => selectTemplate(id)} aria-pressed={template === id} className={`group text-left transition-transform hover:-translate-y-0.5 ${template === id ? "-translate-y-0.5" : ""}`}><div className={`rounded-2xl p-1 transition-colors ${template === id ? "bg-ink" : "bg-canvas-soft group-hover:bg-surface-pressed"}`}><TemplatePreview template={id} image={images[0]} accentColor={accentColor} exactPreview={template === id ? previewUrl : null} /></div><div className="px-1 pt-2"><p className="text-sm font-bold text-ink">{FLYER_TEMPLATE_LABELS[id]}</p><p className="mt-0.5 text-[11px] leading-snug text-mute">{FLYER_TEMPLATE_DESCRIPTIONS[id]}</p></div></button>)}
                  </div>
                ...
                <section className="order-2 border-t border-hairline pt-6">
                  <div className="mb-3"><p className="text-xs font-black uppercase tracking-[0.2em] text-ink">02 / Fotos</p><p className="mt-1 text-sm text-body">Selecciona hasta 6. El orden define cómo aparecen.</p></div>
                  {images.length > 0 ? <div className="grid grid-cols-4 gap-2 sm:grid-cols-6">{images.map((url, index) => <button key={url} type="button" onClick={() => toggleImage(url)} aria-pressed={selectedImages.includes(url)} className={`relative aspect-square overflow-hidden rounded-xl border-2 transition-all ${selectedImages.includes(url) ? "border-ink ring-2 ring-ink/10" : "border-transparent opacity-55 hover:opacity-100"}`}><img src={url} alt={`Foto ${index + 1}`} className="h-full w-full object-cover" />{selectedImages.includes(url) && <span className="absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-ink text-white"><Check className="h-3 w-3" /></span>}<span className="absolute bottom-1 left-1 rounded-full bg-black/60 px-1.5 py-0.5 text-[10px] font-bold text-white">{selectedImages.indexOf(url) + 1 || ""}</span></button>)}</div> : <div className="rounded-2xl bg-canvas-soft p-4 text-sm text-body">Esta propiedad no tiene fotos. Usaremos una composición tipográfica.</div>}
                </section>
